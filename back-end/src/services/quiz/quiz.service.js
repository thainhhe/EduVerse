...
const quizServices = {
  ...
  createQuiz: async (quizData) => {
    try {
      ...
      // extra guard: ensure any provided scope ids are valid ObjectId (redundant to validator but safe)
      ["courseId", "moduleId", "lessonId"].forEach((f) => {
        if (
          validatedData[f] &&
          !mongoose.Types.ObjectId.isValid(validatedData[f])
        ) {
          const err = new Error("Validation failed");
          err.isValidation = true;
          err.validationErrors = [
            { field: f, message: `${f} is not a valid id` },
          ];
          throw err;
        }
      });
