...
const forumService = {
    ...
    async getForumById(id) {
        try {
            const forum = await Forum.findById(id).populate("courseId", "name code description");

            if (!forum) {
                return {
                    status: STATUS_CODE.NOT_FOUND,
                    message: FORUM_ERROR_MESSAGE.FORUM_NOT_FOUND || "Không tìm thấy forum",
                };
            }
    ...
    async getForumByCourseId(courseId) {
        ...
        try {
            ...
            if (!forum) {
                return {
                    status: STATUS_CODE.NOT_FOUND,
                    message: FORUM_ERROR_MESSAGE.FORUM_NOT_FOUND || "Not found forum",
                };
            }
    ...
    async updateForum(id, data) {
        try {
            ...
            if (!forum) {
                return {
                    status: STATUS_CODE.NOT_FOUND,
                    message: FORUM_ERROR_MESSAGE.FORUM_NOT_FOUND || "Not found forum to update",
                };
            }
    ...
    async deleteForum(id) {
        try {
            ...
            if (!forum) {
                return {
                    status: STATUS_CODE.NOT_FOUND,
                    message: FORUM_ERROR_MESSAGE.FORUM_NOT_FOUND || "Not found forum to delete",
                };
            }
