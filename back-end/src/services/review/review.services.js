...
const reviewServices = {
    ...
    try {
      ...
      return {
        ...
        data: reviews.map((review) =>
          reviewHelper.formatReviewForResponse(review)
        ),
      };
    } catch (error) {
  ...
  createReview: async (reviewData) => {
    try {
      ...
      // Validate data
      const validatedData = reviewValidator.validateReviewData(
        cleanedData,
        false
      );
  ...
  getAllReviewsOfCourseAndAvgRating: async (courseId) => {
    try {
      const result = await reviewRepository.getAllReviewsOfCourseAndAvgRating(
        courseId
      );
      return {
        status: systemEnum.STATUS_CODE.OK,
      ...
      console.error(
        "Error in getAllReviewsOfCourseAndAvgRating service:",
        error
      );
      throw new Error(error);
    }
