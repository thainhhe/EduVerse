...
const lessonService = {
    ...
    markLessonCompleted: async (lessonId, userId) => {
        try {
            ...
            const quizzes = await Quiz.find({ lessonId: lessonId, isPublished: true }).select("_id").lean();

            if (quizzes.length > 0) {
                // Kiểm tra tất cả quiz song song
                const passedResults = await Promise.all(
                    quizzes.map(async (quiz) => {
                        const attempts = await scoreRepository.getUserAttempts(userId, quiz._id);
                ...
                console.log("passedResults.every(Boolean)", passedResults.every(Boolean));
                // Nếu bất kỳ quiz nào chưa pass
                if (!passedResults.every(Boolean)) {
                    return {
                        status: 400,
                        message: "You must pass all quizzes in this lesson before marking it as completed.",
                        data: null,
                    };
                }
            }
