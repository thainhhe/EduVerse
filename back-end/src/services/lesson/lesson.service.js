...
const lessonService = {
  ...
  getLessonByModuleId: async (id) => {
    try {
      ...
      return {
        status: system_enum.STATUS_CODE.OK,
        message: lesson_enum.LESSON_MESSAGE.GET_DATA_SUCCESS,
        data: lessonHelper.sortLessonsByOrder(
          result.map((r) => lessonHelper.formatLessonData(r))
        ),
      };
    } catch (error) {
      throw new Error(error);
    ...
    try {
      ...
      if (!lesson)
        return {
          ...
          message: lesson_enum.LESSON_MESSAGE.NOT_FOUND,
        };
      return {
        status: system_enum.STATUS_CODE.OK,
  ...
  //     try {
  //       console.log(
  //         `🎯 Service: Attempting to mark lesson ${lessonId} for user ${userId}`
  //       );
  ...
  //       if (associatedQuiz) {
  //         console.log(
  //           `Lesson ${lessonId} has an associated quiz: ${associatedQuiz._id}`
  //         );

  //         // 1.2. Nếu có quiz, kiểm tra xem user đã "pass" quiz này chưa
  //         const attempts = await scoreRepository.getUserAttempts(
  //           userId,
  //           associatedQuiz._id
  //         );
  //         console.log(
  //           `User ${userId} attempts for quiz ${associatedQuiz._id}:`,
  //           attempts
  //         );
  //         const hasPassed = attempts.some(
  //           (attempt) => attempt.status === "passed"
  //         );
  //         console.log(`User ${userId} has passed quiz: ${hasPassed}`);

  //         if (!hasPassed) {
  //           console.warn(
  //             `User ${userId} tried to complete lesson ${lessonId} but has not passed the quiz.`
  //           );
  //           // TRẢ VỀ LỖI - KHÔNG CHO HOÀN THÀNH
  //           return {
  //             status: system_enum.STATUS_CODE.BAD_REQUEST, // 400
  //             message:
  //               "Bạn phải vượt qua bài quiz trong bài học này trước khi đánh dấu hoàn thành.",
  //             data: null,
  //           };
  //         }
  ...
  //       const result = await lessonRepository.markLessonCompleted(
  //         lessonId,
  //         userId
  //       );
  ...
  //           throw new Error(
  //             "Lesson or Module not found while recalculating progress"
  //           );
  //         }
  ...
  markLessonCompleted: async (lessonId, userId) => {
    try {
      ...
      // --- BƯỚC 1: KIỂM TRA QUIZ TRONG LESSON (nếu có) ---
      const quizzes = await Quiz.find({ lessonId: lessonId, isPublished: true })
        .select("_id")
        .lean();

      if (quizzes.length > 0) {
        // Kiểm tra tất cả quiz song song
        const passedResults = await Promise.all(
          quizzes.map(async (quiz) => {
            ...
            console.log(`User ${userId} passed quiz ${quiz._id}:`, hasPassed);
            return hasPassed;
          })
        );
        console.log("Passed results for all quizzes:", passedResults);
        console.log(
          "passedResults.every(Boolean)",
          passedResults.every(Boolean)
        );
        // Nếu bất kỳ quiz nào chưa pass
        if (!passedResults.every(Boolean)) {
          console.log("Vô ddaaayyy");
          return {
            status: 400,
            message:
              "Bạn phải vượt qua tất cả các quiz trong bài học này trước khi đánh dấu hoàn thành.",
            data: null,
          };
        }
      }

      // Nếu tất cả quiz đã pass, đánh dấu lesson hoàn thành
      const result = await lessonRepository.markLessonCompleted(
        lessonId,
        userId
      );
  ...
  unmarkLessonCompleted: async (lessonId, userId) => {
    try {
      ...
      // 1. Unmark lesson
      const result = await lessonRepository.unmarkLessonCompleted(
        lessonId,
        userId
      );
  ...
  checkLessonCompletion: async (lessonId, userId) => {
    try {
      const isCompleted = await lessonRepository.isLessonCompletedByUser(
        lessonId,
        userId
      );
      return {
