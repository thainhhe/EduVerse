...
const enrollmentRepository = {
        ...
        try {
            ...
            const enrollments = await Enrollment.find({ userId })
                .populate({
                    path: "courseId",
                    populate: [
                        // populate fields that actually exist on Course
                        { path: "main_instructor", select: "name email" },
                        { path: "instructors.user", select: "name email" },
                        { path: "category" },
                    ],
                })
                .populate("userId", "name username email")
            ...
            const result = enrollments.map((en) => {
                ...
                const totalQuizzes =
                    allLessonQuizzes.length + allModuleQuizzes.length + courseQuizList.length;
    ...
    getDetailedEnrollmentByUserIdCourseId: async (userId, courseId) => {
        try {
            ...
            const lessonQuizzes = await Quiz.find({ lessonId: { $in: lessonIds } })
                .select("title passingScore questions lessonId isPublished")
                .lean();
            ...
            const moduleQuizzes = await Quiz.find({
                ...
                .select("title passingScore questions moduleId isPublished")
                .lean();
            ...
            const courseQuizzes = await Quiz.find({
                ...
                .select("title passingScore questions courseId isPublished")
                .lean();
            ...
            const totalQuizzes =
                allLessonQuizzes.length + allModuleQuizzes.length + courseQuizWithScores.length;
            const completedQuizzes =
                allLessonScores.filter((s) => s.status === "passed").length +
                allModuleScores.filter((s) => s.status === "passed").length +
            ...
            const allScores = [
                ...new Set(
                    [...allLessonScores, ...allModuleScores, ...allCourseQuizScores].map((s) =>
                        JSON.stringify(s)
                    )
                ),
            ].map((s) => JSON.parse(s));
            ...
            const averageScore =
                allScores.length > 0
                    ? Math.round(
                          allScores.reduce((acc, s) => acc + (s.percentage || 0), 0) / allScores.length
                      )
                    : 0;
    ...
    getProgress: async (userId, courseId) => {
        return await Enrollment.findOne({ userId, courseId }).select("progress status lastAccessed").exec();
    },
};
