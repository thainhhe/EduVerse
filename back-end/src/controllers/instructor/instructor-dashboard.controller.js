...
const instructor_dashboard = {
    getDashboardAnalytics: async (req, res) => {
        try {
            ...
            if (!instructorId) return res.status(400).json({ success: false, message: "Thiếu instructorId" });
            ...
            const courseIds = await Course.find({ main_instructor: instructorId }).distinct("_id");
            ...
            const videoGrowth = lastMonthVideos === 0 ? 0 : ((totalVideos - lastMonthVideos) / lastMonthVideos) * 100;
            ...
            const enrollmentGrowth =
                lastMonthEnrollments === 0
                    ? 0
                    : ((totalEnrollments - lastMonthEnrollments) / lastMonthEnrollments) * 100;
            ...
            const revenueGrowth =
                lastMonthRevenue === 0 ? 100 : ((totalRevenue - lastMonthRevenue) / lastMonthRevenue) * 100;
            ...
            const revenueDistribution = await Course.aggregate([
                ...
                },
                {
                    $group: {
                        _id: "$category",
                        revenue: { $sum: { $multiply: ["$price", { $size: "$enrollments" }] } },
                    },
                },
                {
                    $lookup: {
