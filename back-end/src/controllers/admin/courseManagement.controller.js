...
} = require("../../services/admin/courseManagement.services");
...
const adminCourseManagementController = {
  ...
  getCourseDetailsById: async (req, res) => {
    try {
      ...
      const result = await courseManagementService.getCourseDetailsById(
        courseId
      );
      return response(res, result);
    } catch (error) {
  ...
  rejectCourse: async (req, res) => {
    try {
      ...
      const result = await courseManagementService.rejectCourse(
        courseId,
        reasonReject
      );
      return response(res, result);
