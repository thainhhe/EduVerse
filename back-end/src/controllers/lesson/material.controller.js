  ...
  async uploadMaterial(req, res) {
    try {
      ...
      if (!bodyData.lessonId) {
        return res
          .status(400)
          .json({ success: false, message: "Missing lessonId" });
      }
      ...
      const material = await Material.create({
        ...
        url: viewUrl,
        // Use driveFile.mimeType (file after Drive processing) to determine type
        type:
          driveFile.mimeType && driveFile.mimeType.startsWith("video/")
            ? "video"
            : "document",
        fileId: driveFile.id,
        fileName: driveFile.name,
        fileSize: parseInt(driveFile.size, 10) || 0,
