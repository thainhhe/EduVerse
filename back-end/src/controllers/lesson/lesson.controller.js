...
const lessonController = {
  ...
  markLessonCompleted: async (req, res) => {
    try {
      ...
      // Nếu service trả về lỗi (status !== 200)
      if (result.status && result.status !== 200) {
        console.log("Error in markLessonCompleted:", result);
        return error_response(res, {
  ...
  unmarkLessonCompleted: async (req, res) => {
    try {
      ...
      const result = await lessonService.unmarkLessonCompleted(
        lessonId,
        userId
      );
      return response(res, {
      ...
      return error_response(res, {
        status: 500,
        message: "Failed to unmark lesson completion",
      });
    }
  },
  ...
  checkLessonCompletion: async (req, res) => {
    try {
      ...
      const result = await lessonService.checkLessonCompletion(
        lessonId,
        userId
      );
      return response(res, {
