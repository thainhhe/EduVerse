...
quizSchema.pre("validate", function (next) {
    const refs = [this.courseId, this.moduleId, this.lessonId].filter((v) => v != null && v !== "");
    if (refs.length > 1) {
        return next(new Error("Quiz must be attached to at most one of courseId, moduleId, or lessonId."));
    }
    next();
});
