...
const Learning = () => {
    ...
    useEffect(() => {
        const fetchCourseDetail = async () => {
            try {
                const data = await enrollmentService.getDetailCourseEnrollmentsByUser(user._id, courseId);
                if (data) {
                    setCourseData(data);
    ...
    useEffect(() => {
        const fetchPassed = async () => {
            ...
            for (let quiz of allQuizzes) {
                ...
                if (res.success && res.data.hasCompleted && res.data.latestScore.status == "passed")
                    passedIds.push(quiz._id);
            }
                        ...
                        {quizStatus.hasCompleted ? (
                            ...
                            <QuizContent
                                ...
                                onQuizSubmitted={(statusData) => {
                                    ...
                                    if (
                                        statusData.hasCompleted &&
                                        statusData.latestScore.status === "passed"
                                    ) {
