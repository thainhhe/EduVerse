...
const Basics = ({ courseId = null, isUpdate = false, courseData = null }) => {
  ...
  const { user } = useAuth() || {};
  const { draft, updateDraft, clearDraft, isMainInstructor, permissions } =
    useCourse();
  ...
  const isEditable =
    isNew || (!!isMainInstructor && courseData?.status === "draft");
  ...
  // Normalize description: trim edges and collapse multiple spaces into single space
  ...
  const handleNext = async () => {
    if (!isEditable) {
      navigate(`/create-course/${visibleNavItems[0].path}`, {
        state: { id: courseId },
      });
      return;
    }
    ...
    // Title validation
    const trimmedTitle = normalizedTitle;
    ...
    if (price === "" || Number(price) < 0)
      ...
      newErrors.price = "Price is too large. Max is 999.999.999";

    // Duration validation
    if (durationValue !== "" && Number(durationValue) < 0)
      newErrors.duration = "Duration must be at least 0";

    // Description validation
    const trimmedDescription = normalizedDescription;
