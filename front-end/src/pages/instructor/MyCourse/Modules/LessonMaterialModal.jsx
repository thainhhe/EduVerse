...
const LessonMaterialModal = ({ open, onOpenChange, lessonId }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                console.log("res-lesson-material:", res);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-9/12 p-6 overflow-y-auto max-h-[80vh]">
                <DialogHeader>
                        ...
                        {materials.map((item) => (
                            <div key={item._id} className="border p-3 rounded-lg">
                                ...
                                {item.type === "video" ? (
                                    ...
                                    <iframe src={item.url} className="w-full h-[500px] rounded-lg border" />
                                )}
                            </div>
                        ))}
                    </div>
