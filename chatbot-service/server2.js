...
console.log("[Chatbot] using model:", "models/gemini-2.5-pro", " embeddings:", "models/text-embedding-004");
    ...
    getRelevantDocuments: async () => {
        try {
            ...
            if (typeof generalRetriever.getRelevantDocuments === "function") {
                const candidates = await generalRetriever.getRelevantDocuments("tổng quan các khóa học");
...
cron.schedule("*/15 * * * *", async () => {
    console.log(`[Cron] ⏰ Triggering auto-sync at ${new Date().toISOString()}...`);
    try {
...
app.post("/query", async (req, res) => {
    try {
        ...
        const isCourseIntent = courseKeywords.some((kw) => lowerCaseMessage.includes(kw));

        if (isCourseIntent) {
            console.log("[Chatbot] Course intent detected -> combining SUMMARY + GENERAL retrievers.");
            ...
            try {
                ...
                console.log(`[Chatbot] General retriever returned ${docs?.length ?? 0} docs`);
            } catch (err) {
        ...
        if (!docs || docs.length === 0) {
            console.log("[Chatbot] No docs found, using direct model fallback stream");
            chainToStream = fallbackPrompt.pipe(model).pipe(new StringOutputParser());
            context = "";
        } else {
            console.log(`[Chatbot] Docs found, using RAG stream (${docs.length} docs)`);
            context = formatDocs(docs);
            chainToStream = ragPromptTemplate.pipe(model).pipe(new StringOutputParser());
        }
...
function isCourseIntentText(text, knownCategories = []) {
    ...
    const verbs = ["học", "dạy", "đào tạo", "tuyển sinh", "đăng ký", "ghi danh", "hoc"];
    const askPhrases = ["có ... không", "có gì", "bạn có", "nào", "tìm khóa", "tìm kiếm khóa"];
    const domains = [
        "công nghệ thông tin",
        "it",
        "lập trình",
    ...
    if (tokens.length > 3 && (direct.some((d) => t.includes(d)) || domains.some((d) => t.includes(d)))) {
        score += 0.5;
    }
...
app.listen(PORT, () => {
    ...
    runSync()
        ...
        .catch((err) => console.error("[Startup] Initial runSync failed:", err?.message || err));
});
...
app.post("/trigger-sync", (req, res) => {
    ...
    if (!expectedKey || !incomingKey || String(incomingKey) !== String(expectedKey)) {
    ...
    try {
        ...
        console.error("[Trigger-Sync] Failed to start runSync:", err?.message || err);
    }
